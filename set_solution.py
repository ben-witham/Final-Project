...
def set_differences(set_of_difference, other_set):
    # This can be done easily with the "-" operator, but to understand everything behind the operator let's make it by hand
    leftover_num_set = set()
    for data in other_set:
        if data not in set_of_difference:
            leftover_num_set.add(data)
    
    return leftover_num_set
...
def is_subset(megaset,subset):
    return megaset.issubset(subset)
